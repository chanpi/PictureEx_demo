	...
	: CDialog(CPictureExDemoDlg::IDD, pParent)
{
	...
	m_hCursor = AfxGetApp()->LoadCursor(IDC_MYHAND);
}
...
BOOL CPictureExDemoDlg::OnInitDialog()
{
	...
	if (pSysMenu != NULL)
	{
		...
		if (!strAboutMenu.LoadString(IDS_ABOUTBOX)) {
			OutputDebugString(_T("LoadString Failed. Return.\n"));
			return FALSE;
		}
	...
	LPCTSTR szFileName = _T("res\\Loading.gif");
	if (__argc >= 2) {
		szFileName = CString(__argv[1]);
	}
	if (m_Picture.Load(szFileName)) {
		m_Picture.Draw();
	}
...
void CPictureExDemoDlg::OnTimer(UINT nIDEvent)
{
	PostQuitMessage(0);
}
